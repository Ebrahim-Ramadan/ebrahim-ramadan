            ...
            {data.map((repo) => (
                <div key={repo.id} className='w-full border border-t-0 border-b-1 border-l-0 border-r-0 border-white/10 p-2 flex justify-between '>
                    <div className='flex flex-col gap-2 '>
                        <a href={repo.clone_url} className='hover:underline text-[#4493F8] font-medium'>{repo.name.length > 40 ? repo.name.substring(0, 40) + '...' : repo.name}</a>
                        ...
                        <p
                            ...
                            >{repo.language}</p>
                            <p className='text-xs  text-gray-400 text-elipsis'>Updated at {repo.updated_at.substring(0, 10)}</p>
                            </div>
                    </div>
